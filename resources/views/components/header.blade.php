<div class="flex justify-between items-center mb-10">
    ...
    <div class="flex items-center gap-4">
        <div id="header-status-indicator" class="flex items-center gap-2 bg-[#121214] px-4 py-2 border border-white/5 rounded transition-all" 
             style="border-color: {{ isset($shopStatus) && $shopStatus === 'open' ? '#10b981' : '#ef4444' }}">
            <span id="header-status-dot" class="w-2 h-2 rounded-full" style="background-color: {{ isset($shopStatus) && $shopStatus === 'open' ? '#10b981' : '#ef4444' }}"></span>
            <span id="header-status-text" class="text-xs font-bold" style="color: {{ isset($shopStatus) && $shopStatus === 'open' ? '#10b981' : '#ef4444' }}">
        ...
        <div class="relative group">
            ...
            <!-- DROPDOWN MENU -->
            <div class="absolute right-0 mt-2 w-48 bg-[#0f0f0f] border border-white/10 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                <div class="px-4 py-3 border-b border-white/5">
                    <p class="text-sm text-gray-400">Signed in as</p>
                    <p class="text-white font-semibold">{{ Auth::user()->email ?? 'Shop Admin' }}</p>
                </div>
                <a href="/shop/settings" class="block px-4 py-2 text-gray-300 hover:text-white hover:bg-white/5 transition">
                    Settings
                </a>
...
<script>
// Listen for status changes from sidebar or dashboard
...
// Also listen for manual API calls
