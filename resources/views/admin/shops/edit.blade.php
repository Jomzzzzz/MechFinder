...
@section('content')
    <div class="mb-8">
        <div>
            <div class="text-[#F4B942] text-2xl heading-font">Edit Shop</div>
            <div class="text-[#AAA] text-sm mt-1">Update shop information and location.</div>
        </div>
    </div>
        ...
        <div class="bg-[#111113] border border-[#1E1E21] rounded-xl p-6 space-y-6">
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input id="address" name="address" type="text" value="{{ old('address', $shop->address) }}"
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input id="latitude" name="latitude" type="text" value="{{ old('latitude', $shop->latitude) }}"
                        class="w-full rounded-lg border border-[#1E1E21] bg-[#0F1115] px-4 py-3 text-[#EEE]">
                </div>
                <div>
                    <label class="block text-sm text-[#AAA] mb-2" for="longitude">Longitude</label>
                    <input id="longitude" name="longitude" type="text" value="{{ old('longitude', $shop->longitude) }}"
                        class="w-full rounded-lg border border-[#1E1E21] bg-[#0F1115] px-4 py-3 text-[#EEE]">
                </div>
            </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input id="phone" name="phone" type="text" value="{{ old('phone', $shop->phone) }}"
                        class="w-full rounded-lg border border-[#1E1E21] bg-[#0F1115] px-4 py-3 text-[#EEE]">
                </div>
                <div>
                    <label class="block text-sm text-[#AAA] mb-2" for="email">Email</label>
                    <input id="email" name="email" type="email" value="{{ old('email', $shop->email) }}"
                        class="w-full rounded-lg border border-[#1E1E21] bg-[#0F1115] px-4 py-3 text-[#EEE]">
                </div>
            </div>
                ...
                <select id="status_id" name="status_id"
                    ...
                    @foreach ($shopStatuses as $status)
                        <option value="{{ $status->id }}"
                            {{ (int) old('status_id', $shop->status_id) === (int) $status->id ? 'selected' : '' }}>
                            {{ $status->label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="text-right">
                <button type="submit"
                    class="bg-[#F4B942] text-[#0A0A0B] px-5 py-3 rounded-lg font-semibold hover:bg-[#e6b12a]">Save Changes</button>
            </div>
        </div>
    </form>
