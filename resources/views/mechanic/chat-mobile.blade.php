    ...
    <div class="min-h-screen flex flex-col bg-[#06090f] text-slate-100">
        ...
        <div class="sticky top-0 z-20 bg-[#08101a]/95 border-b border-slate-800/70 px-4 pt-4 pb-4 backdrop-blur-xl">
            <div class="flex items-center justify-between gap-3 mb-3">
                <a href="{{ route('mechanic.messages') }}"
                    class="w-10 h-10 rounded-2xl bg-slate-900/80 border border-slate-800 text-slate-200 flex items-center justify-center transition hover:bg-slate-800">
                    ←
                </a>
        ...
        <div class="px-4 pb-5 pt-3 border-t border-slate-800/80 bg-[#08101a] sticky bottom-0">
            <form id="chat-form" class="flex items-center gap-3">
                <input type="text" id="message-input" placeholder="Write a message"
                    ...
                    required>
                <button type="submit"
                    class="w-12 h-12 rounded-full bg-orange-500 text-slate-950 text-lg flex items-center justify-center transition hover:bg-orange-400">→</button>
            </form>
            <div id="send-error" style="display:none; font-size:12px; color:#f87171; margin-top:4px; padding:0 4px;"></div>
        </div>
    </div>
        ...
        `;
            ...
            function renderMessages(messages) {
                ...
                if (!messages || messages.length === 0) {
                    chatBox.innerHTML =
                        '<div class="flex h-full min-h-[240px] items-center justify-center"><div class="rounded-[28px] border border-slate-800/80 bg-slate-950/80 px-5 py-4 text-sm text-slate-400">No messages yet. Start the conversation with your motorist.</div></div>';
                    return;
                }
                ...
                fetch('/api/mechanic/messages/send', {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            messageInput.value = '';
                            // Append sent message immediately so it shows even if loadMessages is in-flight
                            if (data.message) appendMessage(data.message);
                            loadMessages();
                        } else {
                            errEl.textContent = data.message || 'Failed to send message.';
