...
@section('content')
    ...
    <div style="display:flex; gap:5px; margin-bottom:8px;">
        <button id="tab-motorist" onclick="switchShopTab('motorist')"
            style="flex:1; padding:5px 5px; border:1px solid #e6e7eb; border-radius:8px; background:#fff; color:#1d273b; font-weight:700; cursor:pointer;">Motorist
            Chats</button>
        <button id="tab-mechanic" onclick="switchShopTab('mechanic')"
            style="flex:1; padding:5px 5px; border:1px solid #e6e7eb; border-radius:8px; background:#f4f6fb; color:#6c7280; font-weight:700; cursor:pointer;">Mechanic
            Chats</button>
    </div>
    ...
    <div class="t-card shop-messages-card">
            ...
            <div id="motorist-list">
                @forelse($conversations as $conv)
                    <div onclick="openChat({{ $conv->dispatch_id }}, '{{ addslashes($conv->motorist_name) }}', 'motorist')"
                        id="conv-motorist-{{ $conv->dispatch_id }}" data-unread="{{ $conv->unread_count ?? 0 }}"
                        ...
                        <div style="display:flex; align-items:center; gap:10px;">
                            <div
                                style="width:36px; height:36px; background:#206bc4; border-radius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                                <span
                                    style="font-size:13px; font-weight:700; color:#fff;">{{ strtoupper(substr($conv->motorist_name ?? 'U', 0, 1)) }}</span>
                            </div>
                            <div style="min-width:0; flex:1;">
                                <p
                                    style="font-size:13px; font-weight:600; color:#1d273b; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                    {{ $conv->motorist_name ?? 'Unknown Motorist' }}</p>
                                <p
                                    style="font-size:12px; color:#a0a8b1; margin:2px 0 0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                    {{ ucfirst(str_replace('_', ' ', $conv->issue_type ?? 'New request')) }}</p>
                            </div>
                        </div>
                ...
                @empty
                    <div style="padding:32px 16px; text-align:center;">
                        <i class="fas fa-comment-slash"
                            style="font-size:28px; color:#c8ccd0; display:block; margin-bottom:8px;"></i>
            ...
            <div id="mechanic-list" class="hidden">
                @forelse($mechanicConversations as $conv)
                    <div onclick="openChat({{ $conv->dispatch_id }}, '{{ addslashes($conv->mechanic_name) }}', 'mechanic')"
                        id="conv-mechanic-{{ $conv->dispatch_id }}" data-unread="{{ $conv->unread_count ?? 0 }}"
                        style="padding:12px 16px; cursor:pointer; border-bottom:1px solid #f0f2f5; transition:background .1s; position:relative;"
                        onmouseover="this.style.background='#f4f6fb'" onmouseout="this.style.background='';">
                        <div style="display:flex; align-items:center; gap:10px;">
                            <div
                                style="width:36px; height:36px; background:#f97316; border-radius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                                <span
                                    style="font-size:13px; font-weight:700; color:#fff;">{{ strtoupper(substr($conv->mechanic_name ?? 'M', 0, 1)) }}</span>
                            </div>
                            <div style="min-width:0; flex:1;">
                                <p
                                    style="font-size:13px; font-weight:600; color:#1d273b; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                    {{ $conv->mechanic_name ?? 'Mechanic' }}</p>
                                <p
                                    style="font-size:12px; color:#a0a8b1; margin:2px 0 0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                    {{ ucfirst(str_replace('_', ' ', $conv->issue_type ?? 'New request')) }}</p>
                            </div>
        ...
        <div style="flex:1; display:flex; flex-direction:column; min-width:0;">
            ...
            {{-- Empty state --}}
            <div id="chat-empty"
                style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#a0a8b1;">
            ...
            <div id="chat-active" class="shop-chat-panel" style="height:100%; min-height:0; overflow:hidden;">
                <div
                    style="padding:12px 18px; border-bottom:1px solid #e6e7eb; display:flex; align-items:center; gap:10px; flex-shrink:0; background:#fff; z-index:1;">
                    <div
                        style="width:34px; height:34px; background:#206bc4; border-radius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                ...
                <div id="chat-messages"
                    style="flex:1 1 0; min-height:0; overflow-y:auto; padding:16px; display:flex; flex-direction:column; gap:8px; box-sizing:border-box;">
                </div>
                <div
                    style="padding:12px 16px; border-top:1px solid #e6e7eb; display:flex; gap:8px; flex-shrink:0; background:#fff; position:sticky; bottom:0; z-index:2;">
                    <input id="msg-input" type="text" placeholder="Type a message..." class="form-control"
                        style="flex:1;" onkeydown="if(event.key==='Enter')sendMsg()">
                    <button onclick="sendMsg()" class="btn btn-primary" style="flex-shrink:0;"><i
    ...
    `;
        ...
        function subscribeDispatchChannel() {
            ...
            if (channel) {
                window.Echo.leave(`dispatch.${activeDispatchId}`);
            }
            ...
            channel.listen('.message.sent', (event) => {
                ...
                if (activeConversationType && event.message?.conversation_type && event.message
