...
<div class="min-h-screen flex flex-col text-white">
    ...
    <div class="px-4 pt-4 pb-6 space-y-4 flex-1 overflow-y-auto">
        ...
        <div>
            ...
            <div id="gps-box" class="glass-card rounded-[16px] px-4 py-3 text-[13px] text-gray-300 flex items-center gap-2">
        ...
        <div>
            ...
            <div class="space-y-2">
                <button type="button" class="issue-btn w-full text-left glass-card rounded-[16px] px-4 py-3 flex items-center gap-3 transition-all" data-issue="Flat Tire / Blowout">
                    <span class="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center text-[18px]">🛞</span>
                    <span class="text-[13px] font-medium">Flat Tire / Blowout</span>
                </button>

                <button type="button" class="issue-btn w-full text-left rounded-[16px] px-4 py-3 flex items-center gap-3 border border-orange-500 bg-orange-500/15 transition-all" data-issue="Engine Stall">
                    <span class="w-10 h-10 rounded-lg bg-orange-500/20 flex items-center justify-center text-[18px]">⚙️</span>
                    <span class="text-[13px] font-semibold text-orange-300">Engine Stall</span>
                    <span class="text-orange-300 ml-auto text-[12px]">✓</span>
                </button>

                <button type="button" class="issue-btn w-full text-left glass-card rounded-[16px] px-4 py-3 flex items-center gap-3 transition-all" data-issue="Oil / Fluid Leak">
                    <span class="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center text-[18px]">🛢️</span>
                    <span class="text-[13px] font-medium">Oil / Fluid Leak</span>
                </button>

                <button type="button" class="issue-btn w-full text-left glass-card rounded-[16px] px-4 py-3 flex items-center gap-3 transition-all" data-issue="Battery / Electrical">
                    <span class="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center text-[18px]">🔋</span>
        ...
        <div>
            ...
            <div class="space-y-2">
                ...
                <div>
                    <label class="block text-[11px] text-gray-500 mb-2 font-semibold">CONTACT NUMBER (SMS/WHATSAPP) *</label>
                    <input
                        type="tel"
                        id="contact_number"
                        class="w-full glass-card rounded-[16px] px-4 py-3 text-[13px] text-white placeholder:text-gray-600 outline-none focus:border-orange-500"
        ...
        <div>
            ...
            <div class="space-y-2">
                ...
                <div>
                    <label class="block text-[11px] text-gray-500 mb-2 font-semibold">LICENSE PLATE / TEMP NUMBER</label>
                    <input
                        type="text"
                        id="plate_temp_number"
                        class="w-full glass-card rounded-[16px] px-4 py-3 text-[13px] text-white placeholder:text-gray-600 outline-none focus:border-orange-500"
    ...
    <div class="px-4 pb-5 pt-3 border-t border-white/5 bg-[#090909]/95 sticky bottom-0">
        <button id="submit-dispatch" type="button" class="w-full rounded-[16px] px-5 py-4 danger-btn text-[14px] font-extrabold tracking-wide transition-all hover:shadow-lg">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    submitBtn.addEventListener('click', function () {
        ...
        // Save motorist info to localStorage for profile display
        if (ownerName) localStorage.setItem('mf_owner_name', ownerName);
        if (contactNumber) localStorage.setItem('mf_contact_number', contactNumber);
        if (vehicleMakeModel) localStorage.setItem('mf_vehicle_make_model', vehicleMakeModel);
        if (vehicleVariantColor) localStorage.setItem('mf_vehicle_variant_color', vehicleVariantColor);
        if (plateTempNumber) localStorage.setItem('mf_plate_temp_number', plateTempNumber);
...
function getGuestIdentity() {
    let guestToken = localStorage.getItem('mf_guest_token');
    let guestName = localStorage.getItem('mf_guest_name');

    if (!guestToken) {
        guestToken = 'MF-' + Date.now() + '-' + Math.random().toString(36).substring(2, 8).toUpperCase();
        localStorage.setItem('mf_guest_token', guestToken);
    }

    if (!guestName) {
        guestName = 'Guest Motorist';
        localStorage.setItem('mf_guest_name', guestName);
    }
