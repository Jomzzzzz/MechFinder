...
<body class="overflow-x-hidden antialiased">
    ...
    <!-- NAV -->
    <nav class="sticky top-0 z-50 border-b border-[#E8E6E1] bg-[#FAFAF8]/90 backdrop-blur-xl">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">

            <!-- Logo -->
            <a href="/" class="flex items-center gap-2.5 flex-shrink-0">
                <div class="w-8 h-8 rounded-xl bg-[#0D0D0D] flex items-center justify-center text-base">⚙️</div>
                <span class="font-bold text-base tracking-tight" style="font-family:Syne,sans-serif">MechFinder</span>
            </a>
            ...
            <div class="nav-desktop items-center gap-0.5 flex-1 justify-center">
                ...
                <a href="#how-it-works"
                    class="px-3.5 py-2 text-sm font-medium text-[#6B6963] hover:text-[#0D0D0D] hover:bg-[#F5F4F1] rounded-lg transition-all">How
                    It Works</a>
                <a href="{{ route('signup') }}"
                    class="px-3.5 py-2 text-sm font-medium text-[#6B6963] hover:text-[#0D0D0D] hover:bg-[#F5F4F1] rounded-lg transition-all">For
                    Motorists</a>
            ...
            <div class="nav-desktop items-center gap-2 flex-shrink-0">
                <a href="{{ route('login') }}"
                    ...
                    in</a>
                <a href="{{ route('signup') }}"
                    class="btn bg-[#0D0D0D] text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-[#2a2a2a] transition-all">Get
    ...
    <!-- HERO -->
    <section
        class="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 md:py-16 lg:py-20 grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center min-h-[calc(100vh-60px)] sm:min-h-[calc(100vh-68px)]">

        <!-- LEFT CONTENT -->
        <div class="flex flex-col justify-center">
            <!-- Badge -->
            <div
                class="inline-flex items-center gap-2 bg-[#F5F4F1] border border-[#E8E6E1] rounded-full px-3 sm:px-3.5 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium text-[#6B6963] uppercase tracking-widest mb-4 sm:mb-6 w-fit">
            ...
            <div class="flex flex-col sm:flex-row flex-wrap gap-2.5 sm:gap-3 mb-8 sm:mb-10">
                <a href="{{ route('signup') }}"
                    class="btn bg-orange text-white font-medium px-5 sm:px-7 py-3 sm:py-3.5 rounded-full hover:bg-[#d97a0f] text-center text-sm sm:text-base">I'm
                    a Motorist &rarr;</a>
                <a href="{{ route('signup.shop') }}"
                    class="btn border-[1.5px] border-[#E8E6E1] text-[#0D0D0D] font-medium px-5 sm:px-7 py-3 sm:py-3.5 rounded-full hover:bg-[#F5F4F1] hover:border-[#aaa] transition text-center text-sm sm:text-base">I'm
                    a Shop Owner</a>
            </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                <div class="flex">
                    @foreach (['MR', 'JL', 'AP'] as $i)
                        <span
                            class="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#E8E6E1] border-2 border-[#FAFAF8] -ml-2 first:ml-0 flex items-center justify-center text-[9px] sm:text-[10px] font-semibold text-[#6B6963]">{{ $i }}</span>
                    @endforeach
                </div>
                <p class="text-xs sm:text-sm text-[#6B6963]"><strong
                        class="text-[#0D0D0D] font-semibold">5,000+</strong> motorists already on MechFinder</p>
            </div>
        </div>
        ...
        <div class="hidden lg:flex justify-center relative h-[400px] md:h-[500px] lg:h-[600px]">
            ...
            <!-- Top Stat Card -->
            <div
                class="absolute top-[8%] -right-2 lg:-right-4 bg-white border border-[#E8E6E1] rounded-xl lg:rounded-2xl px-3 lg:px-4 py-2 lg:py-3 shadow-lg z-10 w-max">
                <p
                    class="text-[8px] lg:text-[10px] uppercase tracking-widest text-[#6B6963] font-medium mb-0.5 lg:mb-1">
                    Avg. Response</p>
                <p class="text-xl lg:text-2xl font-extrabold tracking-tight" style="font-family:Syne,sans-serif">4 min
                </p>
            ...
            <!-- Bottom Stat Card -->
            <div
                class="absolute bottom-[12%] -left-2 lg:-left-4 bg-white border border-[#E8E6E1] rounded-xl lg:rounded-2xl px-3 lg:px-4 py-2 lg:py-3 shadow-lg z-10 w-max">
                <p
                    class="text-[8px] lg:text-[10px] uppercase tracking-widest text-[#6B6963] font-medium mb-0.5 lg:mb-1">
                    Active Shops</p>
                <p class="text-xl lg:text-2xl font-extrabold tracking-tight" style="font-family:Syne,sans-serif">150+
                </p>
        ...
        <div class="lg:hidden flex justify-center relative h-[300px] sm:h-[400px] md:h-[450px] mt-6 sm:mt-8">
            ...
            <div
                class="absolute top-[5%] -right-1 sm:right-2 bg-white border border-[#E8E6E1] rounded-lg px-2.5 sm:px-3 py-2 shadow-md z-10 w-max">
                <p class="text-[7px] sm:text-[8px] uppercase tracking-widest text-[#6B6963] font-medium mb-0.5">Avg.
                    Response</p>
                <p class="text-lg sm:text-xl font-extrabold tracking-tight" style="font-family:Syne,sans-serif">4 min
                </p>
            ...
            <div
                class="absolute bottom-[8%] -left-1 sm:left-2 bg-white border border-[#E8E6E1] rounded-lg px-2.5 sm:px-3 py-2 shadow-md z-10 w-max">
                <p class="text-[7px] sm:text-[8px] uppercase tracking-widest text-[#6B6963] font-medium mb-0.5">Active
                    Shops</p>
                <p class="text-lg sm:text-xl font-extrabold tracking-tight" style="font-family:Syne,sans-serif">150+
                </p>
        ...
        <div class="max-w-6xl mx-auto flex flex-wrap justify-center gap-x-6 sm:gap-x-10 gap-y-2.5 sm:gap-y-3">
            @foreach (['Live Mechanic Tracking', 'Verified Repair Shops', 'Olongapo City Coverage'] as $item)
                <span class="flex items-center gap-2 text-xs sm:text-sm text-[#6B6963] font-medium">
                    <svg class="text-[#F7941D] w-3.5 h-3.5 sm:w-[15px] sm:h-[15px]" fill="none"
    ...
    <section id="features" class="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16 md:py-24">
            ...
            @foreach ([['icon' => '🏍', 'title' => 'For Motorists', 'sub' => 'Get back on the road fast', 'items' => [['📍', 'Find Nearby Shops', 'Discover verified repair shops closest to your exact location in seconds.'], ['🚨', 'Emergency Dispatch', 'Tap once to request urgent roadside assistance wherever you are.'], ['💬', 'Live Chat', 'Describe your problem and get real-time guidance before arrival.'], ['⭐', 'Ratings & Reviews', 'Rate services, read feedback, and make informed choices every time.']]], ['icon' => '🛠', 'title' => 'For Shop Owners', 'sub' => 'Grow your customer base', 'items' => [['📩', 'Receive Requests', 'Get notified instantly for jobs near your shop — never miss a customer.'], ['🟢', 'Real-time Availability', 'Toggle open/closed status and manage your capacity on the fly.'], ['📊', 'Analytics Dashboard', 'Monitor jobs completed, average ratings, and revenue over time.'], ['👨‍🔧', 'Mechanic Management', 'Assign and dispatch your team to customer locations with one tap.']]]] as $card)
                <div class="card bg-white border border-[#E8E6E1] rounded-xl sm:rounded-2xl p-5 sm:p-8">
                    <div class="flex items-start gap-3 sm:gap-4 pb-4 sm:pb-6 mb-4 sm:mb-6 border-b border-[#F5F4F1]">
                        <div
                            class="w-11 h-11 sm:w-13 sm:h-13 bg-[#F5F4F1] rounded-lg sm:rounded-xl flex items-center justify-center text-xl sm:text-2xl flex-shrink-0">
                            {{ $card['icon'] }}</div>
                        <div>
                    ...
                    <ul class="space-y-3 sm:space-y-5">
                        @foreach ($card['items'] as [$icon, $title, $desc])
                            <li class="flex gap-2 sm:gap-3">
                                <div
                                    class="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-orange/10 flex items-center justify-center text-xs sm:text-sm flex-shrink-0 mt-0.5">
                                    {{ $icon }}</div>
                                <div>
                                    <h4 class="text-xs sm:text-sm font-semibold mb-0.5">{{ $title }}</h4>
                                    <p class="text-[11px] sm:text-xs text-[#6B6963] leading-relaxed">
                                        {{ $desc }}</p>
                                </div>
                            </li>
                        @endforeach
        ...
        <!-- STATS -->
        <div
            class="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-[#E8E6E1] border border-[#E8E6E1] rounded-xl sm:rounded-2xl overflow-hidden">
            @foreach ([['150+', 'Repair Shops'], ['5K+', 'Motorists Served'], ['24/7', 'Always Available']] as [$num, $label])
                <div class="bg-white py-6 sm:py-10 px-4 sm:px-6 text-center">
                    <p class="text-4xl sm:text-5xl font-extrabold orange tracking-tight leading-none mb-2"
                        style="font-family:Syne,sans-serif">{{ $num }}</p>
    ...
    <div class="bg-[#0D0D0D] py-12 sm:py-16 md:py-24 px-4 sm:px-6">
        <div class="max-w-2xl mx-auto text-center">
            ...
            <div class="flex flex-col sm:flex-row flex-wrap justify-center gap-2.5 sm:gap-3">
                <a href="{{ route('signup') }}"
                    class="btn bg-white text-[#0D0D0D] font-medium px-6 sm:px-7 py-3 sm:py-3.5 rounded-full hover:bg-[#F5F4F1] transition text-sm sm:text-base">Sign
                    Up as Motorist &rarr;</a>
                <a href="{{ route('signup.shop') }}"
                    class="btn border-[1.5px] border-white/20 text-white/70 font-medium px-6 sm:px-7 py-3 sm:py-3.5 rounded-full hover:border-white/50 hover:text-white transition text-sm sm:text-base">Register
                    Your Shop</a>
            </div>
        </div>
    </div>
    ...
    <footer class="bg-[#F5F4F1] border-t border-[#E8E6E1] py-4 sm:py-6 px-4 sm:px-6">
        <div class="max-w-6xl mx-auto flex flex-col sm:flex-row flex-wrap items-center justify-between gap-3 sm:gap-4">
            <a href="/" class="flex items-center gap-2.5">
                <div
                    class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-[#0D0D0D] flex items-center justify-center text-xs sm:text-sm">
                    ⚙️</div>
                <span class="font-bold tracking-tight text-sm sm:text-base"
                    style="font-family:Syne,sans-serif">MechFinder</span>
            </a>
            <p class="text-xs sm:text-sm text-[#6B6963] text-center sm:text-left">© 2026 MechFinder. Fast motorcycle
